...
try {
   ...
   error_reporting(0); foreach($_POST['delete_row'] as $selected){
    ...
    if(empty($ids_to_delete)){// вывод сообщения с помщью javasctipt и редирект(перемещение) на преддущую страницу admin-panel.php
    ...
    if(sizeof($ids_to_delete > 0)){
        ...
        $sql = "DELETE FROM teachers WHERE id IN (" . implode(',', array_map('intval', $ids_to_delete)) . ")";
...
catch(PDOException $e) {// вывод сообщения с помщью javasctipt и редирект(перемещение) на преддущую страницу admin-panel.php
    echo "<script type='text/javascript'>
    function Show_message() {
...
$db = null;
?>
      <script type='text/javascript'> // В случает успешного выполнения выполняем JavaScript код делающий редирект(перемещение) на предыдущую страницу admin-panel.php
      function Redirect_to_admin() {
        location = "../admin-panel.php";
      }
