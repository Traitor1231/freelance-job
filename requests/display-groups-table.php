...
try {
    ...
    $sql = "SELECT name, image, age, description, teacher, id FROM groups LIMIT 15";
    // Подготовка запроса
    ...
    echo "<form action='requests/delete_groups.php' method='POST'><table>";
    echo "<tr><th class='column-1'>Название группы</th><th class='column-2'>Изображение</th><th class='column-3'>Возрастной диапазон</th><th class='column-4'>Девиз</th><th class='column-5'>Педагог</th><th class='column-6'>ID</th><th class='column-7'>Удалить запись</th></tr>";
    foreach ($result_array as $result_row) {
        echo "<tr>";
        echo "<td>" . $result_row["name"] . "</td>";
        echo "<td>" . $result_row["image"] . "</td>";
        echo "<td>" . $result_row["age"] . "</td>";
		echo "<td>" . $result_row["description"] . "</td>";
		echo "<td>" . $result_row["teacher"] . "</td>";
